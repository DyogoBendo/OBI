...
def main(entrada=input, test=False):
    global presente, matrix, grafo, sinal, n, t
    ...
    n = int(entrada())  # número de cidades
    t = int(entrada())  # dinheiro máximo que o sacoleiro possui para gastar
    
    for i in range(n):
        e = entrada().split()  # recebemos a entrada como um vetor
        
        idc = int(e[0])  # íncide que representa a cidade -> É inútil nesse caso, pois consideramos que as cidades serão colocadas em ordem
        presente[i] = int(e[1])  # guardamos o presente da cidade i
        
        tipo = e[2]  # O tipo da cidade A ou B
        sinal[i] = convert_to_number(tipo)  # Convertemos para 0 se for A e 1 se for B, facilitando análises futuras
        
        nv = int(e[3])  # Indica quantas cidades podem ser visitadas a partir da cidade i 
        
        for j in range(4, 4 + nv):
            v = int(e[j])  # cidade que pode ser visitada pela cidade i
